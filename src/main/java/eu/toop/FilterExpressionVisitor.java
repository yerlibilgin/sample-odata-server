  ...
  public Object visitMember(final Member member) throws ExpressionVisitException, ODataApplicationException {
    ...
    if(uriResourceParts.size() == 1 && uriResourceParts.get(0) instanceof UriResourcePrimitiveProperty) {
      ...
      if (uriResourceParts.size() == 2){
        UriResourceComplexProperty p1 = (UriResourceComplexProperty) uriResourceParts.get(0);
        UriResourcePrimitiveProperty p2 = (UriResourcePrimitiveProperty) uriResourceParts.get(1);

        final Property [] l2Found = new Property[1];
        Property level1 = currentEntity.getProperty(p1.getComplexType().getName());
        //TODO might need type checking
        Object level1Value = level1.getValue();
        if (level1Value instanceof ComplexValue){
          ComplexValue cLevel1Value = (ComplexValue) level1Value;
          cLevel1Value.getValue().forEach(level2Property -> {
            if (level2Property.getName().equals(p2.getProperty().getName())){
              l2Found[0] = level2Property;
            }
          });
        }

        return l2Found[0].getValue();
      } else {
        throw new ODataApplicationException("Only depth 2 is implemented.",
